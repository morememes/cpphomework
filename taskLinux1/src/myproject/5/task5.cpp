...
namespace std{
template <class T>
struct iterator_traits<MyOsIterator<T>>{
    typedef output_iterator_tag iterator_category;
    typedef MyOsIterator<T> self_type;
...
int main()
{
  /*  int a = 1;
    ...
    std::iterator_traits<MyOsIterator<int>> s;
    MergeSort(vec.begin(), vec.end());
    //std::ostream_iterator<int> os(std::cout, " ");
    std::copy(vec.begin(), vec.end(), MyOs);*/
   ///std::cout << std::endl;
    // MyOsIterator<float> MyOsF(std::cout);
